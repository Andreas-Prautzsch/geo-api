...
- Wenn deine Umgebung keinen Zugriff auf externe Registry-Images hat, kannst du über `PHOTON_JAR_URL` auf eine eigene Quelle für die Photon-JAR-Datei verweisen (Standard: GitHub-Releases, aktuell `photon-0.7.4.jar`). Mit `PHOTON_JAR_FILE` legst du fest, unter welchem Dateinamen das JAR im Container abgelegt und gestartet wird.
